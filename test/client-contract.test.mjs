...
test('settings expose wallpaper and glass toggle, and custom image failures are explicit', async () => {
  ...
  const refractToggle = findBy(render(), node => node?.props?.['data-liquid-glass-refract'] === '')
  assert.ok(refractToggle)
  assert.equal(refractToggle.props['aria-pressed'], true)
  assert.equal(context.document.body.getAttribute('data-dsh-liquid-glass-refract'), 'on')
  assert.match(clientSource, /\[data-dsh-liquid-glass-refract='off'\] [\s\S]*?backdrop-filter:\s*blur\(var\(--lg-blur-shell\)\) saturate\(var\(--lg-lens-saturate\)\)/)
  findBy(render(), node => node?.props?.['data-liquid-glass-refract'] === '').props.onClick()
  assert.equal(context.document.body.getAttribute('data-dsh-liquid-glass-refract'), 'off')
  findBy(render(), node => node?.props?.['data-liquid-glass-refract'] === '').props.onClick()
  assert.equal(context.document.body.getAttribute('data-dsh-liquid-glass-refract'), 'on')
